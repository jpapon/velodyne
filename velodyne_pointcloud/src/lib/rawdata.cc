...
namespace velodyne_rawdata
{
 ...
 /** @brief convert raw packets to point cloud
   *
   *  @param pkts raw packets to unpack -- usually a full scanner revolution
   *  @param pc shared pointer to organized point cloud (points are not appended)
   */
  void RawData::unpack(const std::vector<velodyne_msgs::VelodynePacket> &pkts,
                       VPointCloud &pc)
  {
    ROS_DEBUG_STREAM("Received packets, time: " << pkts[0].stamp);
    
    /** special parsing for the VLP16 **/
    if (calibration_.num_lasers == 16)
    {
      unpack_vlp16(pkts, pc);
      return;
    }
    
    // Define dimensions of organized output point cloud.
    pc.width  = pkts.size() * SCANS_PER_PACKET / calibration_.num_lasers;
    pc.height = calibration_.num_lasers;
    pc.resize(pc.width * pc.height);
    
    // Process all packets.
    int n_points = 0;    // Number of points read.
    for (int p = 0; p < pkts.size(); p++) {
      const raw_packet_t *raw = (const raw_packet_t *) &pkts[p].data[0];
  ...
  /** @brief convert raw VLP16 packets to point cloud
   *
   *  @param pkts vector of raw packets to unpack -- usually a full scanner revolution
   *  @param pc shared pointer to organized point cloud (points are not appended)
   */
  void RawData::unpack_vlp16(const std::vector<velodyne_msgs::VelodynePacket> &pkts,
                             VPointCloud &pc)
  {
    float azimuth;
    float azimuth_diff;
    ...
    // Define dimensions of organized output point cloud.
    pc.width  = pkts.size() * BLOCKS_PER_PACKET * VLP16_FIRINGS_PER_BLOCK;
    pc.height = calibration_.num_lasers;
    pc.resize(pc.width * pc.height);
